...
censored** rows and with the two excess dispersions distinct, and — for
...
`test-cens-predict.R` covers the `_cens` families’ prediction path on
**censored** rows with `shapexone` and `shapextwo` distinct — the
combination under which exchanging the two inside
