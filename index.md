...
Two families — equidispersed and overdispersed — each with two
constructors, according to whether the first count was recorded on every
row:

|  | both counts observed | first count sometimes unobserved |
|----|----|----|
| **equidispersed** (Poisson latents) | [`bipois()`](https://anhsmith.github.io/bicountbrms/reference/bipois.md) | [`bipois_partialobs()`](https://anhsmith.github.io/bicountbrms/reference/bipois_partialobs.md) |
| **overdispersed** (negative-binomial latents) | [`binegbin()`](https://anhsmith.github.io/bicountbrms/reference/binegbin.md) | [`binegbin_partialobs()`](https://anhsmith.github.io/bicountbrms/reference/binegbin_partialobs.md) |

Each row of that table is one family: both constructors return the same
likelihood and the same post-processing methods, and differ only in
whether the data carries an observation flag. A fully paired design
...
The `_partialobs()` constructors admit rows on which $`y_1`$ was not
observed. On those rows the likelihood is the same joint with $`y_1`$
integrated out over its whole support, so they still inform the shared
component and the second source’s rate rather than being dropped. After
fitting,
...
which means a value known to lie in a set ($`\le c`$, $`\ge c`$, or
within an interval). Here $`y_1`$ is not observed at all. These
constructors were named `_cens` up to 0.9.1 and renamed at 0.10.0 for
that reason; do not combine them with `cens()`.
...
Both families are built by the same **trivariate reduction** (Holgate
1964; Karlis and Ntzoufras 2003): three independent latent counts
...
convert both ways; the map is a bijection. The rate half applies to both
families and both constructors; only the dispersion half is specific to
the negative-binomial one.
...
|----|----|----|----|----|
| Poisson | [`bipois()`](https://anhsmith.github.io/bicountbrms/reference/bipois.md) | none | $`\mathrm{Var}=\text{mean}`$ | margins are **not** overdispersed |
| negative-binomial | [`binegbin()`](https://anhsmith.github.io/bicountbrms/reference/binegbin.md) | `shapes` (shared), `shapexone`/`shapextwo` (private) | $`m + m^2/\phi`$ | margins **are** overdispersed (the usual case) |

With Poisson latents, each component has $`\mathrm{Var}=\text{mean}`$,
so
[`bipois()`](https://anhsmith.github.io/bicountbrms/reference/bipois.md)
cannot represent overdispersed margins and underfits the marginal (and
difference) variance of real count data.
[`binegbin()`](https://anhsmith.github.io/bicountbrms/reference/binegbin.md)
replaces each latent with $`N \sim \text{NB2}(m, \phi)`$ — Stan’s
`neg_binomial_2`, R’s `dnbinom(size = φ, mu = m)` — with mean $`m`$ and
variance $`m + m^2/\phi`$. It carries the extra spread in **scalar**
dispersion dpars: `shapes` $`=\phi_{\text{s}}`$ for the shared count,
and `shapexone`/`shapextwo` $`=\phi_{\text{x}1},\phi_{\text{x}2}`$, one
per private count. The moments become
...
every $`\phi\to\infty`$ the negative-binomials collapse to Poissons and
`binegbin` $`\to`$`bipois`, under either constructor. That limit is not
only a description: it is used as a test (see [Testing](#testing)).

**Two private dispersions, and how to tie them.** The two sources are
different instruments, and there is no reason their source-only excess
must be equally overdispersed, so
[`binegbin()`](https://anhsmith.github.io/bicountbrms/reference/binegbin.md)
estimates one for each. Before 0.10.0 it carried a single `shapex` and
only the partially observed family had the pair, which was the wrong way
round: every row of a fully paired design informs both, so that is the
case in which they are easiest to identify. To impose
...
$`\phi_{\text{x}2}`$ governs the always-observed margin and appears on
both branches, while $`\phi_{\text{x}1}`$ appears only on the matched
branch and is identified by the matched rows alone.
...
reveal this; a marginal one does. Scalar
`shapes`/`shapexone`/`shapextwo` are instead identified from the
aggregate mean–variance relationship across units. The `binegbin.R` file
header gives the detail.

### `bipois_partialobs()` and `binegbin_partialobs()`: partially observed pairs

These constructors admit rows on which $`y_1`$ was **not observed at
all**. Each row carries a second `vint()` integer, a `y1_obs`
$`\in\{0,1\}`$ flag — `1` where both counts were recorded, `0` where the
first was not. Put any non-negative integer in `y1` on those rows (`0`
is the conventional placeholder); the likelihood does not read it. Do
not use `NA`, which brms drops before fitting, taking the row’s observed
$`y_2`$ with it.

- **`y1_obs == 1` (matched):** the full joint lpmf on $`(y_1, y_2)`$ —
  the same function
  [`bipois()`](https://anhsmith.github.io/bicountbrms/reference/bipois.md)/[`binegbin()`](https://anhsmith.github.io/bicountbrms/reference/binegbin.md)
...
thus pools matched and unmatched rows under one likelihood. An unmatched
row is not dropped and is not given a different model: it still informs
the shared component, the second source’s rate and dispersion, and any
group-level effects. What it cannot inform is $`\lambda_1`$, `shapexone`
and the between-source bias, which appear only on the matched branch and
are identified by the matched rows **alone**. A design with 20 matched
rows in 500 will learn them weakly and lean on their priors — worth
knowing before collecting data, not after seeing a posterior. See
...
return a draw and $`\mathrm{E}[y_1 \mid y_2]`$ for **every** row,
matched and unmatched alike: `y1_obs` selects a likelihood branch, not a
prediction. Imputing the unobserved first count conditional on the
observed second one is usually the reason for wanting this.

**Which shape does a stored fit have?** `family$name` is the same either
way, so the flag’s presence is what distinguishes them:
...
Poissons is Poisson, so for `bipois_partialobs` the convolution above
...
with no sum to evaluate. `binegbin_partialobs` has no such shortcut,
because $`\mathrm{NB2} + \mathrm{NB2}`$ is not $`\mathrm{NB2}`$ unless
the two components share a success probability, so it evaluates the
convolution term by term. This also makes `bipois_partialobs` the
analytic reference against which `binegbin_partialobs`’s unmatched
branch is checked.

It sharpens what the unmatched rows can say, too. For
`bipois_partialobs` they see $`\mu`$ and $`\lambda_2`$ only through
their **sum**: they constrain the total rate of the observed margin, not
how it splits between shared and private. Separating the two — and so
estimating the congruence $`f`$ — rests on the matched rows however many
unmatched rows there are.

**The two excess dispersions under partial observation.**
[`binegbin()`](https://anhsmith.github.io/bicountbrms/reference/binegbin.md)
and
[`binegbin_partialobs()`](https://anhsmith.github.io/bicountbrms/reference/binegbin_partialobs.md)
both give each private component its own dispersion — `shapexone`
$`=\phi_{\text{x}1}`$ for $`N_1`$ and `shapextwo` $`=\phi_{\text{x}2}`$
...
Setting $`\phi_{\text{x}1}=\phi_{\text{x}2}`$ recovers the
single-dispersion expressions of earlier versions.

What differs between the two constructors is not the parameters but how
well they are identified. `shapextwo` governs the always-observed margin
and so appears on both branches, and the unmatched rows sharpen it.
`shapexone` appears only on the matched branch, exactly as $`\lambda_1`$
does. Under full pairing that asymmetry vanishes, which is why
[`binegbin()`](https://anhsmith.github.io/bicountbrms/reference/binegbin.md)
— not
[`binegbin_partialobs()`](https://anhsmith.github.io/bicountbrms/reference/binegbin_partialobs.md)
— is the constructor for which two free dispersions are easiest to
estimate.
...
The second count (and, for the `_partialobs()` constructors, the flag)
travels via `vint()` because
...
`vint1 = y2`, `vint2 = y1_obs` in listed order. On unmatched rows the
...
**A fully paired model never mentions the flag.** The plain constructors
declare `vars = c("vint1[n]", "1")`, and `brms` pastes those entries
into the generated Stan call verbatim, so
[`binegbin()`](https://anhsmith.github.io/bicountbrms/reference/binegbin.md)
reaches the same `binegbin_lpmf` with `y1_obs` fixed at `1`:

    binegbin()             target += binegbin_lpmf(Y[n] | ..., vint1[n], 1);
    binegbin_partialobs()  target += binegbin_lpmf(Y[n] | ..., vint1[n], vint2[n]);

That is why there is one Stan function per family rather than two
overloaded ones, and so why the package declares no floor on the Stan
version — function overloading needs Stan 2.29.
...
returns an empty `prior` column for `lamx`, `shapes`, `shapexone` and
`shapextwo`. These are the weakly-identified parameters, and
unregularised the chains explore the flat tail. In this package’s
recovery test, omitting the priors produced a divergent transition and
$`\hat R = 1.0101`$, although all parameters still recovered. Weakly
informative priors are sufficient:
...
Under the symmetric `nlf(shapexone ~ shapexx)` idiom the last two lines
become `prior(normal(2, 1), class = "b", nlpar = "shapexx")` instead —
routing a dpar through a non-linear parameter moves its prior from
`dpar =` to `nlpar =`.
[`bipois()`](https://anhsmith.github.io/bicountbrms/reference/bipois.md)
and
[`bipois_partialobs()`](https://anhsmith.github.io/bicountbrms/reference/bipois_partialobs.md)
have no dispersion dpars, so only the rate lines apply to them.
...
- **`bipois` / `bipois_partialobs`:** the conditional split is closed
  form. Conditioning a sum of independent Poissons on its total gives a
  Binomial, so
...
- **`binegbin` / `binegbin_partialobs`:** a negative-binomial sum has no
  ...
  are exact.
- Under `_partialobs()`, `y1_obs` is **ignored** by both: every row
  (matched and unmatched alike) gets a conditional $`y_1`$, so the
...
`shapexone`, `shapextwo` \| $`\phi_{\text{x}1}`$, $`\phi_{\text{x}2}`$
\| NB2 dispersions of the two private components (`binegbin`) \|  
`y1_obs` (via `vint()`) \| — \| the `_partialobs()` flag: was $`y_1`$
observed? \|
...
**Migrating from 0.9.1 or earlier.** 0.10.0 is a breaking release;
`NEWS.md` carries the full account.

The family surface changed. `binegbin_joint()` became `binegbin_cens()`
at 0.9.0; at 0.10.0 `bipois_cens()` and `binegbin_cens()` became
[`bipois_partialobs()`](https://anhsmith.github.io/bicountbrms/reference/bipois_partialobs.md)
and
[`binegbin_partialobs()`](https://anhsmith.github.io/bicountbrms/reference/binegbin_partialobs.md),
and all seven old names were **removed**, with no deprecation layer. In
source code, substitute the new name.
...
| [`binegbin()`](https://anhsmith.github.io/bicountbrms/reference/binegbin.md) with `shapex` | [`binegbin()`](https://anhsmith.github.io/bicountbrms/reference/binegbin.md) with `shapexone`/`shapextwo` |

[`binegbin()`](https://anhsmith.github.io/bicountbrms/reference/binegbin.md)
gained the second excess dispersion, so `shapex ~ 1` becomes
`shapexone ~ 1, shapextwo ~ 1`, or the tied
`nlf(shapexone ~ shapexx), nlf(shapextwo ~ shapexx), shapexx ~ 1` for
the old single-dispersion model. A prior set with `dpar = "shapex"`
moves to `nlpar = "shapexx"` under the tied form.

**Fits stored under the old names need the old package.** A fit records
its `family$name` permanently and brms builds `log_lik_<name>` from that
string at call time, so a fit made with `binegbin_cens()` or
`binegbin_joint()` needs a version that still defines those methods.
Install 0.9.1 and keep it attached for that fit; nothing you pass to
[`loo()`](https://mc-stan.org/loo/reference/loo.html) or
[`posterior_predict()`](https://mc-stan.org/rstantools/reference/posterior_predict.html)
can route around it, and 0.10.0 deliberately ships no forwarders.

Stored **`binegbin`** fits are the exception and need nothing: they
carry the name this release keeps, and their single `shapex` resolves to
both `shapexone` and `shapextwo` on the current methods, as pre-0.7.0
`lambdaem`/`lambdalb` resolve to `lambdaone`/`lambdatwo`. A package test
pins that exact combination.

One numerical result also changed, at 0.9.0: see
...
**Identifiability under sparse pairing.** $`\lambda_1`$ and the
between-source bias are informed only by the matched (`y1_obs == 1`)
rows — as is `shapexone` for `binegbin_partialobs` — so a fit with few
matched rows will learn them weakly even if the total sample is large.
The unmatched rows add power for the shared and level parameters and for
`shapextwo`, not for the bias. For `bipois_partialobs` the unmatched
rows see $`\mu`$ and $`\lambda_2`$ only through their sum, so the
congruence $`f`$ is likewise a matched-row quantity. Fits that lean on
the bias, on the congruence, or on a difference between the two excess
dispersions should be judged against the matched subset, not the full
$`n`$.
...
- **normalisation** to $`1`$, over the pair for the matched branch and
  over $`y_2`$ for the unmatched one;
...
- the **Poisson-limit reduction** `binegbin` $`\to`$`bipois`, on both
  branches, checked as a limit (the error must shrink as $`\phi`$ grows)
  rather than at a single tolerance;
...
For the partially observed branch it additionally pins the **marginal
identity** ($`\sum_{y_1}`$ of the matched branch equals the $`y_2`$-only
branch), the agreement of the Stan lpmf with the R reference at both
values of the flag, the **conditional-prediction identity**
(`posterior_predict` draws match joint/marginal) on matched **and
unmatched** rows and with the two excess dispersions distinct, and — for
`bipois_partialobs` — that the closed-form unmatched branch equals the
...
rows with `shapexone` and `shapextwo` distinct — the combination under
which exchanging the two inside
[`posterior_predict_binegbin()`](https://anhsmith.github.io/bicountbrms/reference/binegbin.md)
passed every other test in the suite. Its closing comment records what
is still not covered, and why one candidate check is deliberately absent
...
- **Smoke gates** run with the other fitting tests. Each fits once and
  checks convergence (zero divergences, $`\hat R < 1.02`$) and that the
...
Each family exports two constructors, their `_stanvars()`, and one set
of `log_lik_`, `posterior_predict_` and `posterior_epred_` interface
functions.

| Function | Purpose |
|----|----|
| [`bipois()`](https://anhsmith.github.io/bicountbrms/reference/bipois.md) / [`bipois_stanvars()`](https://anhsmith.github.io/bicountbrms/reference/bipois.md) | Joint bivariate Poisson, both counts observed |
| [`bipois_partialobs()`](https://anhsmith.github.io/bicountbrms/reference/bipois_partialobs.md) / [`bipois_partialobs_stanvars()`](https://anhsmith.github.io/bicountbrms/reference/bipois_partialobs.md) | The same, admitting rows where $`y_1`$ was not observed; closed-form unmatched branch |
| [`binegbin()`](https://anhsmith.github.io/bicountbrms/reference/binegbin.md) / [`binegbin_stanvars()`](https://anhsmith.github.io/bicountbrms/reference/binegbin.md) | Joint bivariate negative-binomial (overdispersed margins), a dispersion per private component |
| [`binegbin_partialobs()`](https://anhsmith.github.io/bicountbrms/reference/binegbin_partialobs.md) / [`binegbin_partialobs_stanvars()`](https://anhsmith.github.io/bicountbrms/reference/binegbin_partialobs.md) | The same, admitting rows where $`y_1`$ was not observed |
...
The two constructors of a family return the same `custom_family` name,
so they share one likelihood and one set of interface functions;
`*_partialobs_stanvars()` returns exactly the same object as the plain
one and exists only so that a call reads consistently.
